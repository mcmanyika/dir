...
def auth_view(request):
	...
	if user is not None:
		...
		return HttpResponseRedirect('/stationery')
	else:
...
def visualsDistrict(request):
    ...
    return render_to_response("visuals_by_district.html", {'yearange': year_range, 'minyear': year_range[-1], 'row': row,'full_name': request.user.username }, context_instance=RequestContext(request))
...
def visuals(request):
    ...
    return render_to_response("visuals.html", {'yearange': year_range, 'minyear': year_range[-1],'full_name': request.user.username}, context_instance=RequestContext(request))
...
def home(request):
    ...
    return render_to_response("index.html", {'deathcount': dm, 'victimscount': row, 'incidents': inc,'byyear': yr, 'full_name': request.user.username}, context_instance=RequestContext(request))
